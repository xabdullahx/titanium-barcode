...
public class ScannerActivity extends Activity implements SurfaceHolder.Callback {
	...
	private void decode(final byte[] data, final int width, final int height) {
		...
		new Thread(new Runnable() {
			public void run() {
				...
				// Log.i("Active threads", activeThreads + "/" +
				// param_maxThreads);
	...
	public void handleDecode(MWResult mwResult) {
		...
		if (param_activeParser != MWParser.MWP_PARSER_MASK_NONE && BarcodeScanner.MWBgetResultType() == BarcodeScanner.MWB_RESULT_TYPE_MW
                && !(param_activeParser == MWParser.MWP_PARSER_MASK_GS1 && !mwResult.isGS1)) {

			String s = MWParser.MWPgetJSON(param_activeParser, mwResult.encryptedResult.getBytes());
            if (s != null) {
            	mwResult.text = s;
            }
        }
		...
		data.putExtra("code", mwResult.text);
		data.putExtra("type", mwResult.typeName);
